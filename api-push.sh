...
if [ -z "$1" ] || [ -z "$2" ]; then
  echo "Usage: $0 <path_to_content_directory> <organization>/<workspace> <TFE Address>"
  exit 0
fi
...
tar -zcvf "$UPLOAD_FILE_NAME" --exclude .git* --exclude devops_api_token -C "$CONTENT_DIRECTORY" .
...
UPLOAD_URL=($(curl \
  ...
  https://$TFE_SERVER/api/v2/workspaces/$WORKSPACE_ID/configuration-versions \
